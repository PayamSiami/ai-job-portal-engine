...
router.get("/", protect, authorize(UserRole.ADMIN), async (req, res) => {
    try {
        ...
        const role = getStringQueryParam(req.query.role);
        ...
        const sortOrder = getStringQueryParam(req.query.sortOrder) || "desc";
...
router.patch("/bulk/roles", protect, authorize(UserRole.ADMIN), async (req, res) => {
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : "Failed to bulk update roles";
        res.status(500).json({ error: errorMessage });
    }
});
